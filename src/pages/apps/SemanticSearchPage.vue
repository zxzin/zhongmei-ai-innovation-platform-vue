...
import { Search, SlidersHorizontal, ExternalLink, Image, AlertTriangle, Database } from '@lucide/vue'
  ...
  <section class="structured-workspace semantic-workspace">
    <main class="semantic-main">
      ...
      <section v-if="searched" class="semantic-results">
        ...
        <header><div><span class="eyebrow">召回结果</span><h2>{{ results.length }} 条高相关资料</h2><p>数据来源、排序规则和关键字段均在列表中明确展示。</p></div><div class="semantic-sort-control"><SlidersHorizontal :size="16" /><BaseSelect v-model="sort" :options="sortOptions" aria-label="结果排序" /></div></header>

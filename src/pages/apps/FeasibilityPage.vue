...
const reasoningWidth = ref(null)
const reasoningCollapsed = ref(false)
const isResizing = ref(false)
const figurePreviewOpen = ref(false)
const streamedStepCount = ref(0)
const streamedReportSectionCount = ref(0)
const activeStreamStep = ref(-1)
...
const steps = [
  ...
  {
    id: 'literature', title: '四路资料召回', tool: '已归集 15 项参考资料',
    thought: '问题理解工具已成功拆分出5个核心检索方向，涵盖了定位精度、防爆计算、断网机制、坡度适应和传感器可靠性等关键技术点。接下来按照流程第二步，需要同时从专利、论文、政策和内部资料四路召回候选证据，获取相关技术的现有研究、工程要求与项目经验，为后续的缺陷分析提供依据。',
  },
  {
...
const literatureReferenceGroups = [
  { title: '专利', items: [
...
const layoutClasses = computed(() => ({
  'is-custom-width': reasoningWidth.value !== null && !reasoningCollapsed.value,
  'is-reasoning-collapsed': reasoningCollapsed.value,
  'is-resizing': isResizing.value,
}))
const layoutStyle = computed(() => reasoningWidth.value === null ? {} : { '--feas-reasoning-width': `${reasoningWidth.value}px` })
const streamStatus = computed(() => {
  if (!isStreaming.value) return ''
  if (activeStreamStep.value >= 0) return `正在分析：${steps[activeStreamStep.value].title}`
  return `正在生成报告：第 ${Math.min(streamedReportSectionCount.value + 1, 12)} 节`
})
...
function startStreaming() {
  ...
  streamedReportSectionCount.value = 0
  activeStreamStep.value = -1
  isStreaming.value = true

  steps.forEach((_, index) => {
    streamTimers.push(window.setTimeout(() => {
      streamedStepCount.value = index + 1
      activeStreamStep.value = index
    }, 220 + index * 720))
  })
  streamTimers.push(window.setTimeout(() => { activeStreamStep.value = -1 }, 220 + steps.length * 720))
  Array.from({ length: 12 }, (_, index) => {
    streamTimers.push(window.setTimeout(() => {
      streamedReportSectionCount.value = index + 1
    }, 900 + steps.length * 720 + index * 520))
  })
  streamTimers.push(window.setTimeout(() => { isStreaming.value = false }, 900 + steps.length * 720 + 12 * 520 + 360))
}
function setReasoningWidth(clientX) {
  const bounds = layoutRef.value?.getBoundingClientRect()
  if (!bounds) return
  const requestedWidth = clientX - bounds.left
  if (requestedWidth <= 96) {
    reasoningCollapsed.value = true
    return
  }
  reasoningCollapsed.value = false
  reasoningWidth.value = Math.min(Math.max(Math.round(requestedWidth), 300), Math.max(300, Math.round(bounds.width - 440)))
}
function startReasoningResize(event) {
  if (event.button !== 0 || window.matchMedia('(max-width: 1240px)').matches) return
  isResizing.value = true
  event.currentTarget.setPointerCapture(event.pointerId)
  setReasoningWidth(event.clientX)
}
function resizeReasoning(event) {
  if (isResizing.value) setReasoningWidth(event.clientX)
}
function stopReasoningResize(event) {
...
function toggleReasoning() {
  ...
  if (!reasoningCollapsed.value && reasoningWidth.value === null) reasoningWidth.value = 420
}
...
<template>
  <section class="feas-page">
    ...
    <main v-else class="feas-workbench">
      <div ref="layoutRef" class="feas-layout" :class="layoutClasses" :style="layoutStyle">
        <aside class="feas-reasoning">
          <section class="feas-input-summary"><span>可行性分析·任务方案</span><p>{{ assessment }}</p></section>
          <section class="feas-thinking">
            <header><h2>分析思路</h2><p>每一步均保留过程依据，可按需查看。</p></header>
            <article v-for="(item, index) in steps.slice(0, streamedStepCount)" :key="item.id" class="feas-thinking-step" :class="{ 'is-writing': activeStreamStep === index }">
              <i><Check :size="12" /></i>
          ...
          <section class="feas-followup">
            ...
            <div class="feas-followup__composer"><textarea v-model="followup" placeholder="例如：若 5G 覆盖不足，应优先验证哪些替代方案？" @keydown.ctrl.enter.prevent="sendFollowup" /></div>
            <small v-if="followupSent">已将补充问题纳入本次分析记录。</small>
          </section>
        </aside>
        <div class="feas-resize-handle" role="separator" aria-orientation="vertical" :aria-valuenow="reasoningCollapsed ? 0 : reasoningWidth" :aria-label="reasoningCollapsed ? '左侧分析栏已收起，向右拖动展开' : '拖动调整左侧分析栏宽度，双击收起'" title="拖动调整左栏宽度；双击收起或展开" @pointerdown="startReasoningResize" @pointermove="resizeReasoning" @pointerup="stopReasoningResize" @pointercancel="stopReasoningResize" @dblclick="toggleReasoning" />

        <section class="feas-report-view">
          <header class="feas-report-toolbar"><div><span>{{ selectedDetail ? selectedDetail.title : '可行性分析报告' }}</span><small v-if="isStreaming && !selectedDetail" class="feas-stream-status"><i aria-hidden="true"></i>{{ streamStatus }}</small></div><div><button v-if="selectedDetail && selectedDetail.id !== 'report'" type="button" class="feas-outline" @click="selectedDetail = null">返回报告</button><button type="button" class="feas-export" @click="ui.notify('可行性分析报告已准备导出', 'success')"><Download :size="15" />导出报告</button></div></header>

          <article v-if="selectedDetail && selectedDetail.id !== 'report'" class="feas-detail">
            ...
            <template v-else-if="selectedDetail.id === 'literature'">
              <section class="feas-reference" aria-label="引用资料"><header><h3>引用资料</h3><span>{{ literatureReferenceTotal }} 项参考资料</span></header><nav class="feas-reference-tabs" aria-label="资料分类"><button v-for="group in literatureReferenceGroups" :key="group.title" type="button" :class="{ active: activeLiteratureTitle === group.title }" :aria-selected="activeLiteratureTitle === group.title" @click="activeLiteratureTitle = group.title"><FileText :size="17" /><span>{{ group.title }}</span><b>{{ group.items.length }}</b></button></nav><section class="feas-reference-list" :key="activeLiteratureGroup.title"><article v-for="item in activeLiteratureGroup.items" :key="item[0]"><b>{{ item[0] }}</b><p>{{ item[1] }}</p></article></section></section>
            </template>
...
<article v-else class="feas-report-document feas-report-direct" :class="{ 'is-streaming': isStreaming }"><header class="feas-report-body-title"><h2>煤矿防爆巡检机器人关键技术研究</h2><h3>方案评估总结</h3></header><section v-show="streamedReportSectionCount >= 1" class="feas-stream-chunk"><h3>1. 原方案挑战</h3><div class="feas-report-image-scroll" aria-label="原方案挑战分析图，双击可查看大图" title="双击查看大图" @dblclick="figurePreviewOpen = true"><img src="/feasibility-gap-analysis.png" alt="煤矿井下自主巡检机器人系统原方案挑战与优化方向图谱"></div></section><section v-show="streamedReportSectionCount >= 2" class="feas-stream-chunk"><h3>2. 优化后方案</h3><h4>煤矿井下防爆边缘计算巡检机器人技术方案可行性分析报告</h4><h3>一、方案目标陈述</h3><p>本方案旨在开发一套适用于煤矿井下复杂环境的防爆自主巡检机器人系统，该系统需满足国家煤矿安全规程 GB3836 系列防爆标准要求。核心技术指标包括：实现 3 秒内危险气体告警响应，支持 12 度坡度巷道连续 8 小时作业能力，在弱光粉尘环境下达到 0.3 米定位精度，并具备 5G 断网条件下的数据缓存与安全巡检功能。该方案通过边缘计算架构降低云端依赖，通过多传感器融合技术提升高湿粉尘环境下的系统可靠性，最终实现煤矿井下无人化智能巡检，降低人员安全风险。</p></section><section v-show="streamedReportSectionCount >= 3" class="feas-stream-chunk"><h3>二、设计原理</h3><p>系统采用分布式边缘计算架构，防爆边缘计算终端搭载 ARM Cortex-A72 四核处理器（主频 1.8GHz），配合 NVIDIA Jetson Xavier NX 边缘 AI 模块，算力达 21TOPS。告警响应时间计算公式为：T_response = T_sensor + T_compute + T_alarm，其中传感器采样周期 T_sensor ≤500ms，边缘计算处理时延 T_compute ≤1500ms，告警触发时延 T_alarm ≤1000ms，总响应时间 ≤3000ms。</p><p>定位系统采用视觉 SLAM 与激光雷达紧耦合融合算法，通过扩展卡尔曼滤波（EKF）进行状态估计。定位误差 σ_total 满足：σ_total² = σ_lidar² + σ_vision² + σ_imu²，其中激光雷达误差 σ_lidar ≤0.15m，视觉里程计误差 σ_vision ≤0.20m，IMU 漂移误差 σ_imu ≤0.10m，融合后定位精度达 0.28m，满足 0.3m 指标要求。</p></section><section v-show="streamedReportSectionCount >= 4" class="feas-stream-chunk"><h3>三、材料选择与性能对比分析</h3><p>防爆外壳采用 ZL102 铸造铝合金材质，抗拉强度 ≥295MPa，屈服强度 ≥165MPa，符合 GB3836.1-2010 Exd IIB T4 防爆等级。相比 304 不锈钢（密度 7.93g/cm³），铝合金密度 2.7g/cm³，可减重 65%，有利于爬坡性能。</p><p>激光雷达选用 SICK TiM781 多线激光雷达，扫描频率 15Hz，测距范围 0.05-25m，角分辨率 0.33°，防护等级 IP67，工作温度 -25℃ 至 +60℃，满足井下环境要求。视觉传感器采用海康威视 DS-2CD6626 防爆网络摄像机，配备 Starlight 低照度 CMOS 传感器，最低照度 0.001Lux，支持 120dB 宽动态范围，有效应对井下弱光环境。</p><p>气体传感器组选用催化燃烧式甲烷传感器（响应时间 ≤20s）、电化学 CO 传感器（分辨率 1ppm）和红外 CO₂ 传感器（精度 ±30ppm），三种原理互补提升可靠性。推荐配置为双冗余传感器阵列，当单一传感器数据偏差超过 15% 时触发交叉验证算法。</p></section><section v-show="streamedReportSectionCount >= 5" class="feas-stream-chunk"><h3>四、结构设计方案</h3><p>机器人本体采用履带式底盘结构，外形尺寸 850mm（长）×650mm（宽）×780mm（高），整机重量 ≤180kg。底盘采用双侧独立驱动履带，单侧配置 500W 防爆伺服电机，减速比 1:30，输出扭矩 150N·m。</p><div class="feas-report-formula">12 度坡度所需牵引力：F = mg(sinθ + μcosθ) = 883N；单侧电机输出推力 1063N，双侧总推力 2126N，安全系数 2.4。</div><p>电池舱布置于底盘中部，采用磷酸铁锂防爆电池组，容量 30Ah/48V，能量密度 140Wh/kg。系统平均功率由驱动功率 800W、计算单元 150W 和传感器组 100W 构成，按效率系数折算后满足 8 小时连续作业需求。边缘计算单元安装于 400mm×300mm×200mm 防爆腔体内，配备强制风冷散热系统，热功率密度 ≤0.8W/cm³。</p></section><section v-show="streamedReportSectionCount >= 6" class="feas-stream-chunk"><h3>五、设计验证和对应指标</h3><p>告警响应验证采用标准气体注入法，在距传感器 0.5m 处释放 1% 甲烷标气，通过高速数据采集系统记录从气体接触到声光报警器启动的全过程时间，要求 100 次测试中 95% 样本 ≤3 秒。</p><p>爬坡性能验证在 12 度标准坡道进行 8 小时连续测试，监测电机温升（≤65℃）、电池 SOC 变化曲线及位置保持能力。定位精度验证在 100m×50m 模拟巷道内布置全站仪基准点，机器人沿预设路径行驶，每 10m 记录实际位置与 SLAM 输出位置偏差，要求均方根误差 RMSE ≤0.3m。</p><p>5G 断网测试通过屏蔽室模拟信号中断，验证本地数据缓存容量（≥128GB SSD）及断网恢复后数据完整性。高湿粉尘可靠性测试参照 GB/T 2423.3 标准，在温度 35℃、相对湿度 95%、粉尘浓度 200mg/m³ 环境下连续运行 72 小时，传感器数据波动率 ≤5% 视为合格。</p></section><section v-show="streamedReportSectionCount >= 7" class="feas-stream-chunk"><h3>六、成本评估</h3><p>单台机器人物料成本构成为：防爆底盘及机械结构 8.5 万元，边缘计算硬件平台（含 AI 模块）6.2 万元，激光雷达与视觉传感器组合 4.8 万元，气体传感器阵列（双冗余配置）3.5 万元，防爆电池组 2.8 万元，防爆电气元件及线缆 1.9 万元，其他辅材 0.8 万元，物料总成本 28.5 万元。</p><p>考虑 20% 的组装调试成本及 15% 的研发分摊，单台制造成本约 38.5 万元。按年产 50 台规模测算，模具及工装分摊后单台成本可降至 32 万元。与进口同类产品（售价 80-120 万元）相比，本方案具有显著成本优势，预期市场售价 55-65 万元区间可实现 35% 毛利率，投资回收期约 18 个月。后续通过国产化替代进口传感器及优化结构设计，成本仍有 15%-20% 下降空间。</p></section><div v-show="streamedReportSectionCount >= 8" class="feas-report-divider feas-stream-chunk"><span>方案调研细节</span></div><section v-show="streamedReportSectionCount >= 8" class="feas-stream-chunk"><h2>煤矿井下防爆边缘计算巡检机器人技术可行性分析报告</h2><h3>一、技术方案细化与背景分析</h3><h4>1.1 核心技术思想与应用场景</h4><p>本方案聚焦煤矿井下极端环境的智能巡检系统构建。核心技术架构整合边缘计算分布式处理、防爆机器人平台、多传感器融合定位、弱网环境数据缓存及高湿粉尘环境气体检测五大技术模块。应用场景覆盖煤矿火灾预警、瓦斯监测、设备故障诊断及应急响应，目标实现“无人化、智能化、本质安全型”的井下作业模式。</p><h4>1.2 关键技术特征与理论基础</h4><p>边缘计算架构采用分布式节点部署，基于雾计算理论将数据处理前置至井下终端。已有火灾报警系统实践表明，边缘处理可将响应时间从传统云端架构的 8-12 秒压缩至 3 秒以内，符合煤矿安全规程的紧急告警时限。</p><p>防爆设计遵循双重保护原则：机械隔爆（Exd IIB T4 等级外壳）与电气本安（甲烷浓度 ≥1.5% 时自动断电）。采用加压外壳型结构，内部维持正压 ≥50Pa，阻止可燃气体渗入，同时配置临时防爆室技术解决充电安全隐患。</p><p>多传感器融合定位基于贝叶斯滤波理论构建视觉-激光雷达-IMU 紧耦合模型。定位误差传播方程为：σ²_total = σ²_lidar + σ²_vision + σ²_imu + 2ρ_lv·σ_lidar·σ_vision，其中 ρ_lv 为传感器间相关系数。GICP-SLAM 结合距离权重图（DWM）的方法已在井下场景实现厘米级定位精度，验证了融合算法的有效性。</p><h4>1.3 产业应用价值与局限性</h4><p>市场价值体现在三方面：降低人员伤亡风险；将传统人工巡检 4-6 小时的周期提升为机器人 24 小时不间断作业；持续积累多模态数据以支撑数字孪生矿山建设。</p><p>技术局限包括：防爆认证周期长（18-24 个月），制约产品迭代速度；井下通信受限，5G 信号在金属巷道衰减严重，需依赖本地缓存；粉尘浓度超过 200mg/m³ 时传感器漂移率可达 15%-20%，需采用多原理冗余设计。</p></section><section v-show="streamedReportSectionCount >= 9" class="feas-stream-chunk"><h3>二、多方案对比及权衡分析</h3><h4>2.1 边缘计算架构方案对比</h4><div class="feas-report-table-wrap"><table><thead><tr><th>方案维度</th><th>集中式边缘节点</th><th>分布式边缘网络</th><th>混合云-边架构</th></tr></thead><tbody><tr><td>响应时延</td><td>2.5-3.5s</td><td>1.8-2.8s</td><td>3.5-5.0s</td></tr><tr><td>算力需求</td><td>单节点 ≥30TOPS</td><td>单节点 10-15TOPS</td><td>云端弹性扩展</td></tr><tr><td>可靠性</td><td>单点故障风险</td><td>冗余备份，可用性 99.5%</td><td>依赖网络稳定性</td></tr><tr><td>部署成本</td><td>高</td><td>中</td><td>低</td></tr><tr><td>技术成熟度</td><td>高</td><td>中</td><td>低</td></tr></tbody></table></div><p><b>推荐方案：</b>采用分布式边缘网络。其多区域并行处理可将关键告警路径缩短 40%；单节点故障不影响全局；支持渐进式部署，初期覆盖主巷道，后续扩展至采掘面。</p><h4>2.2 定位技术方案权衡</h4><ul><li><b>方案 A：纯激光 SLAM。</b>不受光照影响、点云精度高，但长走廊退化严重，需进行误差修正。</li><li><b>方案 B：视觉-激光融合。</b>互补性强，视觉提供纹理约束、激光提供尺度信息；弱光环境需要图像增强，计算负载增加约 30%。</li><li><b>方案 C：多模态 BEV 融合。</b>统一特征空间、可动态调整权重，但需大量标注数据训练，工程化难度较高。</li></ul><p>决策选择方案 B，并引入轻量化图像增强。关键参数为：视觉帧率 15fps，激光扫描频率 20Hz，IMU 采样 200Hz，融合周期 50ms，满足 0.3m 精度要求且算力消耗 ≤18TOPS。</p><h4>2.3 气体传感器配置策略</h4><p>单一催化燃烧式甲烷传感器响应时间约 20 秒，且易受硅中毒影响。推荐多原理融合阵列：第一阵列室配置催化燃烧 CH₄、电化学 CO、红外 CO₂；第二阵列室配置 7 通道半导体阵列；融合算法动态调整权重，粉尘遮挡时自动切换至红外通道。</p><p>融合模型准确率可达 96%，较单传感器提升 14 个百分点；高湿环境漂移率从 18% 降至 5%。双阵列配置增加硬件成本约 1.2 万元/台，但故障率降低 60%，全生命周期 TCO 降低约 35%。</p></section><section v-show="streamedReportSectionCount >= 10" class="feas-stream-chunk"><h3>三、理论推导与计算验证</h3><h4>3.1 爬坡能力理论计算</h4><p>煤矿巷道典型坡度 θ=12°，机器人质量 m=180kg，履带-地面摩擦系数 μ=0.3。所需牵引力 F_required = mg(sinθ + μcosθ) = 883N。按减速比、机械效率和履带半径计算，单侧电机输出推力为 3188N，双侧安全系数 K=7.2，大于煤矿设备要求的 2.5。</p><p>坡道功率 P_slope = F_required × v = 441W，加上计算单元 150W 和传感器 100W，总功率 691W，低于 1050W 设计值，续航裕度充足。</p><h4>3.2 定位精度误差分析</h4><p>基于扩展卡尔曼滤波框架，状态向量 X=[x, y, θ, v_x, v_y, ω]ᵀ，观测向量 Z=[z_lidar, z_vision, z_imu]ᵀ。蒙特卡洛仿真 1000 次后，融合定位 RMSE 为 0.28m，95% 置信区间为 [0.22m, 0.34m]。粉尘导致视觉退化时，RMSE 可能升至 0.41m，需要启动纯激光模式。</p><h4>3.3 气体检测响应时间建模</h4><p>多传感器融合检测时延 T_gas = max(T_diffusion, T_sensor, T_fusion)。距泄漏源 0.5m、风速 0.3m/s 时，扩散时延 1.67s；采用红外优先策略时传感器响应约 5s；边缘 AI 融合推理约 0.15s。总时延约 5.15s，虽超过 3 秒目标，但通过将 0.5% CH₄ 设置为黄色预警阈值，可实现更早告警并提升安全裕度。</p></section><section v-show="streamedReportSectionCount >= 11" class="feas-stream-chunk"><h3>四、长远研发与实验设计方案</h3><h4>4.1 分阶段验证路线图</h4><ol><li><b>第一阶段（6 个月）：实验室环境验证。</b>搭建 12° 坡度测试台；开展 50/100/200mg/m³ 粉尘浓度梯度测试；在 0.001-0.1Lux 弱光环境下比较不同融合方案。</li><li><b>第二阶段（12 个月）：矿井模拟巷道试验。</b>在 500m 测试巷道布置全站仪基准点；完成连续 72 小时耐久性测试；模拟 5G 断网并验证 128GB SSD 缓存容量及数据完整性。</li><li><b>第三阶段（18 个月）：实际矿井示范应用。</b>联合 3-5 家煤矿覆盖不同地质条件；建立故障数据库，MTBF 目标 ≥2000 小时；并行推进 MA 认证和防爆合格证。</li></ol><h4>4.2 关键实验装置设计</h4><p>多物理场耦合测试平台包括：温度 -10~45℃、湿度 30%-95%RH 的环境舱；煤尘粒径小于 75μm 的粉尘发生器；CH₄/CO/CO₂ 标准气体配比系统；5-50Hz、0.5-2g 的振动台；以及 100Hz 同步采集传感器原始数据、融合输出和地面真值的数据采集系统。</p><p>风险预控包括：防爆测试严格遵循 GB3836.1-2010 并委托国家级检测机构；电池单体温度超过 60℃ 时强制冷却、超过 80℃ 时应急放电；通信采用 5G 主链路、WiFi6 备份和有线光纤应急通道。</p><h4>4.3 数据分析与迭代优化</h4><div class="feas-report-table-wrap"><table><thead><tr><th>指标类别</th><th>监控参数</th><th>目标值</th><th>数据来源</th></tr></thead><tbody><tr><td>安全性</td><td>防爆等级保持率</td><td>100%（零失效）</td><td>压力传感器 + 甲烷检测器</td></tr><tr><td>可靠性</td><td>定位精度 RMSE</td><td>≤0.3m</td><td>全站仪对比</td></tr><tr><td>实时性</td><td>告警响应时延</td><td>≤3s（95% 分位）</td><td>时间戳日志</td></tr><tr><td>续航</td><td>8 小时作业 SOC 余量</td><td>≥20%</td><td>BMS 数据</td></tr><tr><td>环境适应</td><td>粉尘环境传感器漂移率</td><td>≤5%</td><td>标定气体对比</td></tr></tbody></table></div><p>通过收集实际矿井数据，训练场景自适应模型，实现定位算法 OTA 升级，预期第三代产品定位精度提升至 0.15m。</p></section><section v-show="streamedReportSectionCount >= 12" class="feas-stream-chunk"><h3>五、综合结论与战略建议</h3><h4>5.1 可行性综合评估</h4><p><b>技术可行性：</b>五大核心技术均有成熟案例支撑，防爆认证与极端环境适应等关键风险可通过冗余设计和充分测试规避。定位精度 0.28m、分布式架构响应时延 2.8s，均优于目标指标。</p><p><b>经济可行性：</b>规模化后单台制造成本约 32 万元，售价 55-65 万元，毛利率约 35%，投资回收期约 18 个月；相比进口产品具有显著价格优势。</p><p><b>产业化风险：</b>短期需提前布局防爆认证和国产传感器替代；中期建议采用“租赁 + 运维服务”模式降低客户初始投资；长期需拓展金属矿山、隧道施工等应用场景。</p><h4>5.2 战略建议与技术攻关方向</h4><p><b>近期突破（12 个月内）：</b>完成视觉-激光融合工程化验证并发布 V1.0 定位算法 SDK；突破高湿粉尘环境气体检测技术；启动防爆认证流程。</p><p><b>中期布局（2-3 年）：</b>构建煤矿数字孪生平台；开发集成灭火、自主充电和多机协同调度的第二代产品；推动制定煤矿井下巡检机器人技术规范。</p><p><b>长期愿景（5 年以上）：</b>从“遥控 + 自主”混合模式向完全自主智能体演进；联合矿山装备企业、通信运营商和 AI 芯片厂商构建端-边-云协同生态；拓展海外矿山市场。</p><p><b>关键技术挑战：</b>开发耐高温、抗腐蚀的传感器封装材料；为多模态大模型部署准备 100TOPS 以上边缘算力；制定机器人与矿工的人机协同安全规范。</p><p class="feas-report-conclusion">综上，煤矿井下防爆边缘计算巡检机器人技术方案在理论上可行，工程实现路径清晰，经济效益显著。建议立即启动实验室验证，并行推进防爆认证，逐步实现规模化商业应用。</p></section></article>
        </section>
      </div>

      <div v-if="figurePreviewOpen" class="feas-figure-preview" role="dialog" aria-modal="true" aria-label="原方案挑战分析图大图预览" @click.self="figurePreviewOpen = false"><div class="feas-figure-preview__stage"><button type="button" aria-label="关闭大图预览" @click="figurePreviewOpen = false">×</button><img src="/feasibility-gap-analysis.png" alt="煤矿井下自主巡检机器人系统原方案挑战与优化方向图谱"></div></div>
    </main>
  </section>
</template>
...
.feas-followup__composer { position: relative; }
.feas-followup__composer textarea { box-sizing: border-box; min-height: 96px; }
...
.feas-detail .feas-reference { margin: 0; border: 0; background: transparent; padding: 0; }
.feas-reference > header { display: flex; align-items: baseline; justify-content: space-between; gap: 18px; margin: 0 0 18px; }
.feas-reference > header h3 { margin: 0; color: #173f5a; font-size: 26px; font-weight: 750; line-height: 1.25; letter-spacing: -.02em; }
.feas-reference > header > span { flex: 0 0 auto; color: #7d95a5; font-size: 14px; }
.feas-reference-tabs { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; margin: 0 0 18px; }
.feas-reference-tabs button { display: flex; align-items: center; justify-content: center; gap: 7px; min-width: 0; border: 1px solid #d4e5ed; border-radius: 10px; background: #fff; padding: 11px 10px; color: #547386; font: 700 15px/1 "Microsoft YaHei", sans-serif; cursor: pointer; transition: .16s ease; }
.feas-reference-tabs button svg { flex: 0 0 auto; color: #1d8ac0; }
.feas-reference-tabs button span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.feas-reference-tabs button b { display: grid; place-items: center; flex: 0 0 auto; min-width: 21px; height: 21px; border-radius: 50%; padding: 0 4px; color: #4b768d; background: #e8f3f8; font: 800 12px/1 Arial, sans-serif; }
.feas-reference-tabs button:hover { border-color: #a9d5e6; background: #f2fafc; color: #0c72ac; }
.feas-reference-tabs button.active { border-color: #9ed2e4; background: #e2f4fa; color: #0870aa; box-shadow: inset 0 -3px 0 #1e9fbd; }
.feas-reference-tabs button.active b { background: #1b91bc; color: #fff; }
.feas-detail .feas-reference-list { margin: 0; overflow: hidden; border: 0; border-top: 1px solid #dce8ee; border-radius: 0; background: transparent; padding: 0; }
.feas-reference-list article { display: grid; grid-template-columns: 174px minmax(0, 1fr); gap: 20px; align-items: start; border-bottom: 1px solid #e2ecf1; padding: 17px 14px; transition: background .16s ease; }
.feas-reference-list article:hover { background: #f6fbfd; }
.feas-reference-list article > b { color: #1978aa; font: 800 15px/1.55 Arial, "Microsoft YaHei", sans-serif; }
.feas-reference-list article p { margin: 0; color: #47697e; font-size: 16px; line-height: 1.6; text-align: left; }
.feas-reference-list article:last-child { border-bottom: 0; }
...
@media (max-width: 700px) { .feas-reference > header { align-items: flex-start; gap: 10px; margin-bottom: 14px; }.feas-reference > header h3 { font-size: 23px; }.feas-reference > header > span { padding-top: 5px; font-size: 12px; }.feas-reference-tabs { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; margin-bottom: 14px; }.feas-reference-tabs button { gap: 5px; padding: 10px 7px; font-size: 13px; }.feas-reference-list article { grid-template-columns: 1fr; gap: 5px; padding: 13px 5px; }.feas-reference-list article > b { font-size: 13px; }.feas-reference-list article p { font-size: 14px; } }
.feas-layout { grid-template-columns: minmax(300px, 36%) 16px minmax(0, 1fr); gap: 0; }
.feas-layout.is-custom-width { grid-template-columns: var(--feas-reasoning-width) 16px minmax(0, 1fr); }
.feas-layout.is-reasoning-collapsed { grid-template-columns: 0 16px minmax(0, 1fr); }
.feas-reasoning { min-width: 0; transition: opacity .16s ease; }
.feas-layout.is-reasoning-collapsed .feas-reasoning { visibility: hidden; pointer-events: none; overflow: hidden; padding-right: 0; opacity: 0; }
.feas-resize-handle { position: relative; z-index: 2; display: block; min-height: 100%; cursor: col-resize; touch-action: none; outline: 0; }
.feas-resize-handle::after { position: absolute; top: 10px; bottom: 10px; left: 7px; width: 2px; border-radius: 999px; background: transparent; content: ""; transition: background-color .16s ease, width .16s ease, left .16s ease; }
.feas-resize-handle:hover::after, .feas-layout.is-resizing .feas-resize-handle::after { left: 5px; width: 6px; background: #72b6d8; }
.feas-resize-handle:focus-visible::after { left: 5px; width: 6px; background: #1777b7; }
.feas-layout.is-resizing { user-select: none; }
.feas-report-direct .feas-report-image-scroll { cursor: zoom-in; }
.feas-report-direct .feas-report-image-scroll:hover { border-color: #7dbdde; box-shadow: 0 10px 24px rgba(25, 111, 165, .13); }
.feas-figure-preview { position: fixed; z-index: 3000; inset: 0; display: grid; place-items: center; padding: clamp(18px, 4vw, 56px); background: rgba(8, 25, 39, .72); backdrop-filter: blur(5px); }
.feas-figure-preview__stage { position: relative; display: grid; place-items: center; width: min(1440px, 100%); max-height: 100%; border: 1px solid rgba(192, 224, 239, .65); border-radius: 14px; background: #fff; padding: 16px; box-shadow: 0 28px 76px rgba(0, 0, 0, .3); }
.feas-figure-preview__stage img { display: block; width: auto; max-width: 100%; max-height: calc(100vh - clamp(72px, 9vw, 132px)); height: auto; border-radius: 7px; }
.feas-figure-preview__stage button { position: absolute; top: -13px; right: -13px; display: grid; place-items: center; width: 34px; height: 34px; border: 2px solid #fff; border-radius: 50%; background: #0b71ae; color: #fff; font: 400 26px/1 Arial; cursor: pointer; box-shadow: 0 4px 14px rgba(0, 50, 80, .28); }
.feas-figure-preview__stage button:hover { background: #075886; }
@media (max-width: 1240px) { .feas-layout, .feas-layout.is-custom-width, .feas-layout.is-reasoning-collapsed { grid-template-columns: minmax(0, 1fr); }.feas-layout.is-reasoning-collapsed .feas-reasoning { visibility: visible; pointer-events: auto; overflow: visible; opacity: 1; }.feas-resize-handle { display: none; } }
@media (max-width: 700px) { .feas-report-direct .feas-report-image-scroll img { min-width: 0; }.feas-figure-preview { padding: 18px; }.feas-figure-preview__stage { padding: 9px; }.feas-figure-preview__stage button { top: -10px; right: -7px; width: 31px; height: 31px; font-size: 23px; } }
.feas-thinking-step { animation: feas-stream-in .38s ease both; }
.feas-thinking-step.is-writing > i { background: #1b91bc; box-shadow: 0 0 0 5px rgba(27, 145, 188, .13); animation: feas-stream-pulse 1.25s ease-in-out infinite; }
.feas-thinking-step.is-writing button { border-color: #a9d9ea; background: #eef9fc; }
.feas-report-toolbar > div:first-child { display: grid; min-width: 0; gap: 3px; }
.feas-stream-status { display: inline-flex; align-items: center; gap: 6px; color: #3d829f; font-size: 11px; font-weight: 600; }
.feas-stream-status i { width: 6px; height: 6px; border-radius: 50%; background: #1f9bbc; box-shadow: 0 0 0 0 rgba(31, 155, 188, .45); animation: feas-stream-pulse 1.25s ease-in-out infinite; }
.feas-report-direct.is-streaming > .feas-stream-chunk { animation: feas-report-chunk-in .42s ease both; }
@keyframes feas-stream-in { from { opacity: 0; transform: translateY(7px); } to { opacity: 1; transform: translateY(0); } }
@keyframes feas-report-chunk-in { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
@keyframes feas-stream-pulse { 0%, 100% { box-shadow: 0 0 0 0 rgba(31, 155, 188, .35); } 50% { box-shadow: 0 0 0 6px rgba(31, 155, 188, 0); } }
@media (prefers-reduced-motion: reduce) { .feas-thinking-step, .feas-thinking-step.is-writing > i, .feas-stream-status i, .feas-report-direct.is-streaming > .feas-stream-chunk { animation: none; } }
@media (min-width: 1600px) {
  .feas-workbench { width: min(1840px, calc(100% - 40px)); padding: 30px 0 64px; }
  .feas-layout { grid-template-columns: minmax(430px, 31%) 18px minmax(0, 1fr); }
  .feas-layout.is-custom-width { grid-template-columns: var(--feas-reasoning-width) 18px minmax(0, 1fr); }
  .feas-layout.is-reasoning-collapsed { grid-template-columns: 0 18px minmax(0, 1fr); }
  .feas-resize-handle::after { left: 8px; }
  .feas-resize-handle:hover::after, .feas-layout.is-resizing .feas-resize-handle::after, .feas-resize-handle:focus-visible::after { left: 6px; }
  .feas-reasoning { top: 24px; gap: 16px; max-height: calc(100vh - 48px); padding-right: 6px; }
  .feas-input-summary, .feas-thinking, .feas-followup { border-radius: 14px; }
  .feas-input-summary { padding: 22px; }
  .feas-input-summary > span { font-size: 15px; }
  .feas-input-summary p { margin-top: 9px; font-size: 16px; line-height: 1.8; }
  .feas-thinking { padding: 22px; }
  .feas-thinking header h2 { font-size: 22px; }
  .feas-thinking header p { margin-top: 5px; margin-bottom: 14px; font-size: 14px; }
  .feas-thinking-step { grid-template-columns: 30px minmax(0, 1fr); gap: 13px; padding: 14px 0 16px; }
  .feas-thinking-step:not(:last-child)::before { top: 40px; left: 14px; }
  .feas-thinking-step > i { width: 29px; height: 29px; }
  .feas-thinking-step b { font-size: 17px; }
  .feas-thinking-step p { margin: 7px 0 10px; font-size: 15px; line-height: 1.75; }
  .feas-thinking-step button { padding: 10px 12px; font-size: 14px; }
  .feas-followup { padding: 20px; }
  .feas-followup h3 { font-size: 20px; }
  .feas-followup p, .feas-followup textarea { font-size: 14px; }
  .feas-followup textarea, .feas-followup__composer textarea { height: 110px; min-height: 110px; }
  .feas-report-view { min-height: 740px; }
  .feas-report-toolbar { min-height: 66px; padding: 14px 22px; font-size: 16px; }
  .feas-report-toolbar button { min-height: 40px; padding: 9px 14px; font-size: 14px; }
  .feas-report-document.feas-report-direct { width: min(1240px, calc(100% - 52px)); max-width: none; margin: 22px auto 38px; padding: 52px 70px 82px; }
  .feas-report-direct .feas-report-body-title { margin-bottom: 42px; }
  .feas-report-direct .feas-report-body-title h2 { margin-bottom: 30px; font-size: 40px; }
  .feas-report-direct .feas-report-body-title h3 { font-size: 32px; }
  .feas-report-document.feas-report-direct > section, .feas-report-document.feas-report-direct > section + section { margin-bottom: 42px; }
  .feas-report-direct h2 { margin-bottom: 24px; font-size: 32px; }
  .feas-report-direct h3 { margin-bottom: 18px; font-size: 26px; }
  .feas-report-direct h4 { margin: 24px 0 12px; font-size: 21px; }
  .feas-report-direct p, .feas-report-direct ol, .feas-report-direct ul { font-size: 17px; line-height: 2; }
  .feas-report-direct li + li { margin-top: 9px; }
  .feas-report-direct .feas-report-formula { margin: 20px 0; padding: 13px 17px; font-size: 16px; }
  .feas-report-direct .feas-report-image-scroll { margin: 18px 0 8px; }
  .feas-report-divider { margin: 56px 0 42px; font-size: 28px; }
  .feas-report-table-wrap { margin: 20px 0 22px; }
  .feas-report-table-wrap table { font-size: 15px; }
  .feas-report-table-wrap th, .feas-report-table-wrap td { padding: 14px 16px; }
  .feas-stream-status { font-size: 13px; }
}
</style>

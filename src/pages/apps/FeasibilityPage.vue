    ...
    <main v-else class="feas-workbench">
      <div ref="layoutRef" class="feas-layout" :class="layoutClasses" :style="layoutStyle">
        ...
        <section class="feas-report-view">
          <header class="feas-report-toolbar"><div><span>{{ selectedDetail ? selectedDetail.title : '可行性分析报告' }}</span><small v-if="isStreaming && !selectedDetail" class="feas-stream-status"><i aria-hidden="true"></i>{{ streamStatus }}</small></div><div><button v-if="selectedDetail" type="button" class="feas-outline" @click="selectedDetail = null">返回报告</button><button type="button" class="feas-export" @click="ui.notify('可行性分析报告已准备导出', 'success')"><Download :size="15" />导出报告</button></div></header>

...
const dataDefinitionGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '活跃用户数', formula: '每个趋势时间桶内至少发生 1 次有效行为的去重用户数。', detail: '有效行为包括登录、发起任务、查询或查看历史记录；近 30 天按日、近 90 天按周、本年度按半月统计，同一用户在单个时间桶内多次操作仍只计 1 人。' },
      { label: '用户活跃率', formula: '周期有效行为去重用户数 ÷ 当前组织范围的总注册用户数 × 100%。', detail: '例如：月活跃率 = 当月有效行为去重用户数 ÷ 总注册用户数。' },

...
import { Download, FileText, FileUp } from '@lucide/vue'
...
<template>
  <section class="innovation-page">
    ...
    <section v-else class="inn-wf-shell">
      <div class="inn-wf-layout" :class="{ 'report-flow-collapsed': stage === 'report' && reportWorkflowNavCollapsed }">
        <aside class="inn-wf-nav" :class="{ collapsed: stage === 'report' && reportWorkflowNavCollapsed }">
          <h1 v-show="stage !== 'report' || !reportWorkflowNavCollapsed">创新性分析</h1>
          <button v-if="stage === 'report'" class="inn-wf-nav-collapse" :aria-label="reportWorkflowNavCollapsed ? '展开流程导航' : '收起流程导航'" :title="reportWorkflowNavCollapsed ? '展开流程导航' : '收起流程导航'" type="button" @click="reportWorkflowNavCollapsed = !reportWorkflowNavCollapsed">{{ reportWorkflowNavCollapsed ? '›' : '‹' }}</button>
          <div v-show="stage !== 'report' || !reportWorkflowNavCollapsed">
            <template v-for="(item, index) in workflowSteps" :key="item.id">
              ...
              <nav v-if="item.id === 'research' && stage === 'research'" class="inn-wf-subnav" aria-label="资料检索分类">
                <button type="button" :class="{ active: researchPanel === 'theme' }" @click="researchPanel = 'theme'">技术主题资料</button>
                <button type="button" :class="{ active: researchPanel === 'point' }" @click="researchPanel = 'point'">创新点资料</button>
              </nav>
            </template>
          </div>
        </aside>
        <main class="inn-wf-main">
          <article class="inn-wf-card">
            ...
            <section v-if="stage === 'research'" class="inn-wf-section inn-wf-research-page">
              ...
              <section v-if="researchPanel === 'theme'" class="inn-wf-point-recall">
                <header class="inn-recall-controls">
                  <nav class="inn-recall-type-tabs"><button v-for="source in Object.keys(sourceMeta)" :key="source" :class="{ active: themeSource === source }" type="button" @click="themeSource = source; themeExpanded = ''">{{ sourceMeta[source].label }}</button></nav>
                  <div class="inn-recall-filters"><button v-for="filter in sourceMeta[themeSource].filters" :key="filter" type="button">{{ filter }}</button></div>
                </header>
                <div class="inn-recall-meta"><div><strong>{{ sourceMeta[themeSource].total }}</strong><span>条数据</span><small>技术主题 · 当前展示 {{ activeThemeItems.length }} 条高相关候选</small></div><div><button type="button" @click="selectAll(activeThemeItems, true)">全选</button><button type="button" @click="selectAll(activeThemeItems, false)">清空</button><button class="inn-recall-sort" type="button">最相关</button></div></div>
                <section class="inn-recall-results"><article v-for="(item, index) in activeThemeItems" :key="item.id" class="inn-recall-result" :class="{ 'is-selected': item.selected }"><header><span class="inn-recall-order">{{ String(index + 1).padStart(2, '0') }}</span><div class="inn-recall-title"><div class="inn-wf-theme-membership"><span v-for="topic in item.topics" :key="topic">主题{{ topic === 0 ? '一' : '二' }}</span></div><h3>{{ item.title }}<i>›</i></h3><div class="inn-recall-tags"><span v-for="tag in sourceMeta[themeSource].tags.slice(index % 3, index % 3 + 3)" :key="tag">{{ tag }}</span></div></div><label class="inn-recall-selection"><input v-model="item.selected" type="checkbox" /><span>{{ item.selected ? '已入选' : '待选择' }}</span></label></header><p><b>摘要</b>{{ item.excerpt }}</p><footer><span>{{ item.source }}</span><span>{{ item.meta }}</span><div><button type="button" @click="toggleThemeDetail(item.id)">{{ themeExpanded === item.id ? '收起详情' : '查看详情' }}</button></div></footer><div v-if="themeExpanded === item.id" class="inn-recall-detail"><b>证据摘要</b><p>{{ item.excerpt }}</p><small>请结合原文核验技术特征、适用边界与对应技术主题。</small></div></article></section>
              </section>

              <section v-else class="inn-wf-point-recall">
                <nav class="inn-wf-point-tabs"><button v-for="(point, index) in scopePoints" :key="point" :class="{ active: pointTab === index }" type="button" @click="pointTab = index; pointExpanded = ''"><b>创新点 {{ index + 1 }}</b><span>{{ point }}</span></button></nav>
                <header class="inn-recall-controls"><nav class="inn-recall-type-tabs"><button v-for="source in Object.keys(sourceMeta)" :key="source" :class="{ active: pointSource === source }" type="button" @click="pointSource = source; pointExpanded = ''">{{ sourceMeta[source].label }}</button></nav><div class="inn-recall-filters"><button v-for="filter in sourceMeta[pointSource].filters" :key="filter" type="button">{{ filter }}</button></div></header>
                <div class="inn-recall-meta"><div><strong>{{ pointTotals[pointTab][pointSource] }}</strong><span>条数据</span><small>创新点资料 · 当前展示 {{ activePointItems.length }} 条高相关候选</small></div><div><button type="button" @click="selectAll(activePointItems, true)">全选</button><button type="button" @click="selectAll(activePointItems, false)">清空</button><button class="inn-recall-sort" type="button">最相关</button></div></div>
                <section class="inn-recall-results"><article v-for="(item, index) in activePointItems" :key="item.id" class="inn-recall-result" :class="{ 'is-selected': item.selected }"><header><span class="inn-recall-order">{{ String(index + 1).padStart(2, '0') }}</span><div class="inn-recall-title"><h3>{{ item.title }}<i>›</i></h3><div class="inn-recall-tags"><span v-for="tag in sourceMeta[pointSource].tags.slice(0, 3)" :key="tag">{{ tag }}</span></div></div><label class="inn-recall-selection"><input v-model="item.selected" type="checkbox" /><span>{{ item.selected ? '已入选' : '待选择' }}</span></label></header><p><b>摘要</b>{{ item.excerpt }}</p><footer><span>{{ item.source }}</span><span>{{ item.meta }}</span><div><button type="button" @click="togglePointDetail(item.id)">{{ pointExpanded === item.id ? '收起详情' : '查看详情' }}</button></div></footer><div v-if="pointExpanded === item.id" class="inn-recall-detail"><b>证据摘要</b><p>{{ item.excerpt }}</p><small>请结合原文核验技术特征、适用边界与本创新点的关联。</small></div></article></section>
              </section>
            </section>
            ...
            <footer v-if="stage !== 'report'" class="inn-wf-actions"><button class="inn-wf-back" type="button" @click="previousStage">返回</button><button class="primary-btn" type="button" @click="nextStage">{{ stage === 'research' ? '确认并生成报告' : '确认并进入下一步' }}</button></footer>

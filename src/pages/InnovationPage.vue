...
<template>
  <section class="innovation-workspace">
    ...
    <BaseDrawer :open="Boolean(detail)" title="证据详情" @close="detail = null"><div v-if="detail" class="source-detail"><span class="eyebrow">{{ detail.type }} · {{ detail.id }}</span><h2>{{ detail.title }}</h2><ul><li>正式来源：{{ detail.source }}</li><li>公开日期：{{ detail.date }}</li><li>语义相关度：{{ detail.relevance }}%</li></ul><p>生产系统将展示摘要、申请人／作者、公开信息、权利要求或论文关键段落，并提供原文跳转。</p></div></BaseDrawer>
